...
export default function Results({ product, results, onRestart, onEditMarkets, activeScenario }) {
  const { overallScore, markets } = results;
  ...
  return <div>
    ...
    <div className="results-top">
      <div className="score-hero"><div className="label">Market Success Score</div><div className="value">{overallScore}<span> / 100</span></div><div className="sub">
    Simulated across {markets.length} market{markets.length > 1 ? 's' : ''} for{' '}
    <strong>{product.name}</strong>
    {' '}at {formatCurrency(product.price, product.currency)}
  </div></div>
